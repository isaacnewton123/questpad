...
export default function CheckInSuccessModal({
  ...
  onClose: () => void;
  rewardType?: "coin" | "spin";
  rewardValue?: number;
}) {
  const isSpin = rewardType === "spin";
  const rewardName = isSpin ? "Spin" : "Coins";
  const bgColor = isSpin ? "bg-emerald-100 border-emerald-50" : "bg-amber-100 border-amber-50";
  const iconColor = isSpin ? "text-emerald-500" : "text-amber-500";
  const btnColor = isSpin ? "bg-emerald-500 hover:bg-emerald-600 shadow-emerald-200" : "bg-amber-500 hover:bg-amber-600 shadow-amber-200";
      ...
      <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl relative animate-in zoom-in-95 duration-200">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-slate-500 mb-6">
            You've successfully checked in for today and earned <strong className={iconColor}>{rewardValue} {rewardName}</strong>. Come back tomorrow for more!
          </p>
          <button
            onClick={onClose}
